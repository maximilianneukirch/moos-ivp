...
/*    FILE: BHV_AvdColregsV22Sail.cpp                                */
...
#include "GeomUtils.h"
#include "BHV_AvdColregsV22Sail.h"
#include "AOF_R17.h"
#include "AOF_R16.h"
#include "AOF_R14.h"
#include "AOF_R13.h"
...
BHV_AvdColregsV22Sail::BHV_AvdColregsV22Sail(IvPDomain gdomain) : 
...
bool BHV_AvdColregsV22Sail::setParam(string param, string value) 
{
...
void BHV_AvdColregsV22Sail::onHelmStart()
{
...
void BHV_AvdColregsV22Sail::onIdleState() 
{
...
void BHV_AvdColregsV22Sail::onRunToIdleState() 
{
...
void BHV_AvdColregsV22Sail::onInactiveState() 
{
  if(m_bearing_line_show)
    postViewableBearingLine(false);
}
...
void BHV_AvdColregsV22Sail::onCompleteState() 
{
...
bool BHV_AvdColregsV22Sail::onRunStatePrior() 
{
  return(true);
...
IvPFunction *BHV_AvdColregsV22Sail::onRunState() 
{
  m_iterations++;
...
string BHV_AvdColregsV22Sail::getInfo(string param)
{
  string result;
...
double BHV_AvdColregsV22Sail::getDoubleInfo(string param)
{
  double result = 0;
...
void BHV_AvdColregsV22Sail::updateAvoidMode()
{
  if (!m_wind_valid) {
    return;
  }

  // 1. Check Overtaking First (Rule 13 supersedes Rule 12)
  checkModeOvertaking();
  if (m_avoid_mode == "overtaking" || m_avoid_mode == "standon_ot") {
    return;
  }

  // 2. Determine Tacks
  string os_tack = getTack(m_osh, m_wind_dir);
  string cn_tack = getTack(m_cnh, m_wind_dir);

  // 3. Rule 12(a)(i): Different Tacks
  if (os_tack != cn_tack) {
    if (os_tack == "port") {
  ...
  // 4. Rule 12(a)(ii): Same Tack
  else {
    bool os_is_windward = isWindward(m_osx, m_osy, m_cnx, m_cny, m_wind_dir);
...
void BHV_AvdColregsV22Sail::checkModeOvertaking()
{
  //=====================================================================  
...
IvPFunction* BHV_AvdColregsV22Sail::buildOvertakingIPF()
{
...
void BHV_AvdColregsV22Sail::checkModeHeadOn()
{
  //=====================================================================  
  // Part 1: Sanity checks, and absolute release check based on range
  //=====================================================================  
...
IvPFunction* BHV_AvdColregsV22Sail::buildHeadOnIPF()
{
  // If ownship is already within the min_util_cpa_dist then we shrink
  // this distance, othewise the resulting ivp function will be universally
  // negative, resulting a function with no preference in any direction.
...
void BHV_AvdColregsV22Sail::checkModeGiveWay()
{
  //=====================================================================  
  // Part 1: Sanity checks, and absolute release check based on range
  //=====================================================================  
...
IvPFunction* BHV_AvdColregsV22Sail::buildGiveWayIPF()
{
  // If ownship is already within the min_util_cpa_dist then we shrink
  // this distance, othewise the resulting ivp function will be universally
  // negative, resulting a function with no preference in any direction.
...
void BHV_AvdColregsV22Sail::checkModeStandOn()
{
  //=====================================================================  
  // Part 1: Sanity checks, and absolute release check based on range
  //=====================================================================  
...
void BHV_AvdColregsV22Sail::checkModeStandOnOT()
{
  //=====================================================================  
  // Part 1: Sanity checks, and absolute release check based on range
  //=====================================================================  
...
void BHV_AvdColregsV22Sail::checkModeCPA()
{
...
IvPFunction* BHV_AvdColregsV22Sail::buildStandOnIPF()
{
...
IvPFunction* BHV_AvdColregsV22Sail::buildCPA_IPF()
{
  // If ownship is already within the min_util_cpa_dist then we shrink
  // this distance, othewise the resulting ivp function will be universally
  // negative, resulting a function with no preference in any direction.
...
double BHV_AvdColregsV22Sail::getRelevance() const
{
...
bool BHV_AvdColregsV22Sail::resetAvoidModes(string mode, string submode)
{
  bool valid = true;
...
bool BHV_AvdColregsV22Sail::findDecelerateSpeedRange(double& minv, double& maxv)
{
...
bool BHV_AvdColregsV22Sail::findAccelerateSpeedRange(double& minv, double& maxv)
{
  // Part 0: initialize the results
  minv = 0;
  maxv = 0;
...
void BHV_AvdColregsV22Sail::addHeading(double heading, double currtime)
{
...
void BHV_AvdColregsV22Sail::addRelBng(double rel_bng, double currtime)
{
...
void BHV_AvdColregsV22Sail::clearHeadings()
{
...
void BHV_AvdColregsV22Sail::clearRelBngs()
{
...
bool BHV_AvdColregsV22Sail::getHeadingRate(double& result, double min_secs)
{
  result = 0;
...
bool BHV_AvdColregsV22Sail::getRelBngRate(double& result)
{
  result = 0;
...
void BHV_AvdColregsV22Sail::postStatusInfo()
{
...
string BHV_AvdColregsV22Sail::getTack(double heading, double wind_dir) {
  // Determine tack type (port/starboard) of vessel

  double wind_angle = wind_dir;

  double rel_wind = wind_angle - heading;
  if (rel_wind < 0) rel_wind += 360;

  if (rel_wind > 0 && rel_wind <= 180) {
    return "starboard";
  } else {
    return "port";
  }
...
bool BHV_AvdColregsV22Sail::isWindward(double os_x, double os_y, double cn_x, double cn_y, double wind_dir) {
  // Determine which boat is windward if both boats are on the same tack

  double wind_rad = (90 - wind_dir) * (M_PI / 180.0);
  double wx = cos(wind_rad);
  double wy = sin(wind_rad);

  double dx = cn_x - os_x;
  double dy = cn_y - os_y;

  // Dot product of wind vector and distance vector
  // If positive, the contact is downwind (leeward) of ownship.
  // Meaning ownship is windward.
  return ((wx * dx + wy * dy) > 0);
}
