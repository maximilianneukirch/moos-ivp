...
#ifndef BHV_AVD_COLREGS_V22_SAIL_HEADER
#define BHV_AVD_COLREGS_V22_SAIL_HEADER
...
class IvPDomain;
class BHV_AvdColregsV22Sail : public IvPContactBehavior {
 public:
  BHV_AvdColregsV22Sail(IvPDomain);
  ~BHV_AvdColregsV22Sail() {};
 ...
 protected:
  ...
  void         checkModeSail();                  // Rule-12
  void         checkModeOvertaking();            // Rule-12
  ...
  bool         updateWindInfo();
  std::string  getTack(double heading, double wind_dir);
  bool         isWindward(double os_x, double os_y, double sn_x, double cn_y, double wind_dir);
 ...
 private:  // State Variables
  ...
  double       m_wind_dir;
  std::string  m_wind_var;
  bool         m_wind_valid;
};

extern "C" {
  IvPBehavior * createBehavior(std::string name, IvPDomain domain) 
  {return new BHV_AvdColregsV22Sail(domain);}
}
